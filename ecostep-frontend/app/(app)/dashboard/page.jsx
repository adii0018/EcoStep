...
export default function DashboardPage() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      const { data } = await axios.get(`${process.env.NEXT_PUBLIC_API_URL || 'http://localhost:5000/api'}/activities/summary`, {
      ...
      setSummary({
        ...
        breakdown: data.breakdown || { travel: 9.1, food: 4.8, energy: 3.2, shopping: 1.3 },
    ...
    <div className="space-y-6 pb-12 pt-4 md:pt-0">
      ...
      <div className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 mb-8">
        ...
        <div className="flex flex-wrap items-center gap-3">
          <Link href="/tips" className="flex items-center gap-2 px-4 py-2 bg-white/5 hover:bg-white/10 border border-white/10 text-white font-medium rounded-xl transition-all">
            <Lightbulb className="w-4 h-4 text-emerald-400" />
            Get AI Tips
          </Link>
          <Link href="/log" className="flex items-center gap-2 px-4 py-2 bg-emerald-500 hover:bg-emerald-400 text-zinc-950 font-bold rounded-xl transition-all hover:scale-[1.02] active:scale-[0.98] shadow-[0_0_15px_rgba(16,185,129,0.25)]">
      ...
      <RecentActivity activities={summary.recentActivities} onDeleted={fetchDashboardData} />
    </div>
  );
}
